...
> Deviations require an ADR in the parent plan (`meta-skill-process-architect-plan.md` §17).
...
**Key Design Insight** (spike-01 A1): the refund-process spike's "挽单失败→退款" is NOT an exception — it's an alternative expected path. Use `S{n}.A`, not `S{n}.E`.
...
### Process-Specific Tests (plan §15.2)
...
**When in doubt**: refer to parent plan `meta-skill-process-architect-plan.md`. This archetype is derived from plan §5-§7, §11-§12, §14-§15. The plan is canonical.
