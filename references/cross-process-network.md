...
| `退款处理流程` | Directory whose SKILL.md title contains "退款" |
| `refund-process` | Directory whose process-brief mentions refund-process |
...
```mermaid
graph LR
    REFUND[退款处理<br/>process-refund] -->|parallel| SUPPLY[供应链发货<br/>⚠️ 缺失]
    REFUND -->|downstream| QA[质检判罚<br/>⚠️ 缺失]
    REFUND -->|downstream| STATUS[资格管理<br/>⚠️ 缺失]
    UNLOCK[解锁服务<br/>process-unlock] -->|downstream| REFUND
    unlock-process -->|downstream| DEFER[延期复课<br/>⚠️ 缺失]
    ENROLL[入营注册<br/>process-enrollment] -->|downstream| REFUND
    enrollment-process -->|downstream| DEFER
    enrollment-process -->|parallel| TRIAL[体验课销售<br/>⚠️ 缺失]
