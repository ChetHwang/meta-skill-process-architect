...
在 SKILL.md Step 1.5（Select Input Mode）中，Interview Mode 作为选项之一。Agent 通过 `references/mode-registry.md` 的 trigger_phrases 匹配路由到此模式。

选择 Interview Mode 时：
1. 加载对应脚本模板（`templates/interview-scripts/`）
2. 分别访谈三方（或同时，视可获得性）
3. 按采集→映射表填充 L0+L1
4. 按合并算法处理冲突
5. 对齐验证清单发给 Owner
